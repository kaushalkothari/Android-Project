...
public class DSQUICKSORT extends Activity implements android.view.View.OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_dsquicksort);
	...
	void sortarray( int lower,int higher )
	{
		...
		*/
		int i=lower;
		int j=higher;
		int pivote=number.get(lower)+(number.get(higher-lower)/2);
		while(i<=j)
		{
			while(number.get(i)<pivote) 
				i++;
			while (number.get(j)>pivote)
				j--;
			if(i<=j)
			{
				exchange(i,j);
